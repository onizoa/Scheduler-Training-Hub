...
const startCountdown = () => {
    ...
    // Only start counting down if the required time has not been met
    countdownInterval = setInterval(() => {
        if (!video.paused && !video.ended && watchedTime < requiredTime) {
            ...
            if (watchedTime >= requiredTime) {
                ...
                if (!watchedVideos[currentSrc]) { // Only mark if not already watched
...
const updatePlaylistUI = () => {
    ...
    items.forEach(item => {
        ...
        if (watchedVideos[videoSrc]) {
            ...
            // Show "Completed" badge if watched (THIS IS THE FIX)
            if (completionLabel) completionLabel.classList.remove('hidden');
        } else {
            item.classList.remove('watched');
            // Hide "Completed" badge if not watched
            if (completionLabel) completionLabel.classList.add('hidden');
            allWatched = false;
        }

        // 2. Handle Active/Playing/Paused classes for the visual indicator (Glow/Pause)
        if (item === activeItem) {
            if (video.paused || video.ended) {
                // Paused or Ended: show grey outline (active class)
                item.classList.remove('playing');
            } else {
                // Playing: show red glow
                item.classList.add('playing');
            }
        } else {
            // Non-active items should not have active or playing states
            item.classList.remove('active', 'playing');
        }
    });

    // Re-replace icons after updating innerHTML
    if (typeof feather !== 'undefined') {
        feather.replace();
    }
    
    // Enable or disable assessment button
...
// Mark video as watched when the countdown finishes (or as fallback when video ends)
const videoEndedHandler = (fromCountdown = false) => {
    const currentSrc = video.getAttribute('src');
    
    // Check for completion based on the countdown logic OR if the native 'ended' event fired
    if (fromCountdown || watchedTime >= requiredTime) {
        watchedVideos[currentSrc] = true;
        ...
        // If the native 'ended' event fires but the required watch time hasn't been met, 
        video.pause();
        if (video.duration) {
...
if (playPause) {
    playPause.onclick = () => {
        if (video.paused) {
            ...
            // Only start the countdown if the video isn't already marked as watched
            if (!watchedVideos[video.src]) {
                startCountdown(); // Resume countdown when playing
            }
        } else {
            video.pause();
            playPause.innerHTML = feather.icons.play.toSvg();
            clearInterval(countdownInterval); // Pause countdown when paused
        }
    };
}
...
items.forEach(item => {
    item.onclick = () => {
        ...
        if (canPlay) {
            items.forEach(i => i.classList.remove('active', 'playing')); // Clear all states
            ...
            // --- NEW: Reset and start countdown for the new video ---
            resetCountdown(item);
            
            // Start the countdown only if it's not already watched
            if (!watchedVideos[video.src]) {
                startCountdown();
            }
...
// START: ASSESSMENT POPUP JAVASCRIPT
...
if (openAssessmentBtn) {
    openAssessmentBtn.onclick = (e) => {
        // Recalculate allWatched on click for assessment gate
        const allWatched = Object.keys(watchedVideos).length === TOTAL_VIDEOS;
        ...
        if (allWatched) {
            ...
            e.preventDefault();
            // Fallback alert for disabled button if user somehow bypasses click block
            alert('You must complete all video tutorials before starting the assessment.');
        }
    };
}

if (closeAssessmentBtn) {
    closeAssessmentBtn.onclick = () => {
        assessmentOverlay.classList.remove('active');
    };
}
// END: ASSESSMENT POPUP JAVASCRIPT